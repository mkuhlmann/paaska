...
export const paaskaBuild = async (service: PaaskaService) => {
	if (service.build || service.labels['paaska.build']) {
		...
		if (typeof service.labels['paaska.build'] === 'string') {
			buildPath = path.join(service.project.path, service.labels['paaska.build']);
		} else if (service.build && service.build.context) {
			buildPath = service.build.context;
		} else {
			throw new Error(`Service ${service.name} has no buildPath`);
		}
